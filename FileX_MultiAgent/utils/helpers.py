...
def convert_date_to_dssat_date(date_str: str) -> date:
    ...
    try:
        ...
        return date(2025, 3, 15)  # Default fallback
...
def _load_aez_database(
    ...
    # Country directory must match the normalization used by the writers
    # (generate_dataset.py's build_output_path / cultivar_agent._db_file_path):
    # lowercase with spaces replaced by underscores, e.g. "united_states".
    ...
    if not crop_dir.exists():
        crop_dir = db_path / country_dir_name / crop_code.lower()
...
def _generate_zone_list_for_country(country: str, llm) -> list:
    ...
    country that has no curated entry in data/zones_list.json.

    Best-effort: returns [] if the LLM output cannot be parsed as a JSON
    array of zone name strings.
    """
    prompt = f"""
You are an agronomist. List the standard agro-ecological zones (AEZs)
commonly used for crop planning in {country}.

Rules:
- Return ONLY a JSON array of zone name strings, nothing else.
- Use between 4 and 10 zones.
- Use concise, commonly recognized zone names (e.g. based on altitude,
  rainfall, or climate classification typically used for {country}).

Example format: ["Zone A", "Zone B", "Zone C"]
""".strip()
...
def get_zone_by_location(location_name: str, country: str,model: Optional[str] = None) -> Optional[str]:
    """
    ...
    Returns:
        Optional[str]: One of the AEZ zone names:
             ["Coastal Lowland", "Inner Lowland", "Lower Highland",
              "Lower Midland", "Upper Midland", "Upper Highland",
              "Tropical Alpine", "Nairobi"]
             or None if the zone could not be determined (e.g. zones file
             missing, country not covered by the AEZ database, or the LLM
             did not return a recognizable zone).
    """
    ...
    # Get zones for the detected country. If this country has no curated
    # entry yet, ask the LLM to propose a reasonable AEZ zone list and cache
    # it to zones_file so future lookups for this country are instant and
    # consistent (no re-prompting per location).
    zones = zones_data.get(country, [])

    if not zones:
        print(f"[get_zone_by_location] No cached zones for country: {country}. Generating via LLM...")
        zones = _generate_zone_list_for_country(country, llm)

        if not zones:
            print(f"[get_zone_by_location] LLM could not propose AEZ zones for country: {country}")
            return None

        zones_data[country] = zones
        zones_file.parent.mkdir(parents=True, exist_ok=True)
        with open(zones_file, "w", encoding="utf-8") as f:
            json.dump(zones_data, f, indent=2, ensure_ascii=False)
        print(f"[get_zone_by_location] Cached {len(zones)} LLM-generated zones for '{country}' in {zones_file}")
...
""".strip()
    ...
    # Fallback: LLM did not return a recognizable zone
    print(f"[get_zone_by_location] LLM returned unrecognized zone: {candidate!r}")
    return None
